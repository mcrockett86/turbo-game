...
All 17 components fully implemented + Phase 0 wiring + Phase 4 game flow + Phase 5 audio + Phase 6 visual polish complete (~6,300 lines of code).
...
| 7.5 | **Balance difficulty** — tune presets using `DIFFICULTY_PRESETS` values, playtest each level | `config.ts`, `engine/state.ts` | Game is fun at all levels | ✅ DONE — widened easy↔hard gaps (threatFailPenalty -8→-20, threatSuccessBonus 8→3, comfortItemBonus 1.5→0.8), upgraded DOG_TRAIT_MODIFIERS (Speed 1.25, Brave 1.2, Happiness 1.15, Sniff 1.3, Compact 1.1), added DOG_TRAIT_THREAT_BONUS mapping (each trait helps specific threat types), added CONFIG happiness item tiers (comfort/rare/story items), wired all new fields into state.ts resolveThreat/enterRoom/useItem |
| 7.6 | **Dog trait tuning** — verify each dog trait actually changes gameplay meaningfully | `config.ts`, `engine/state.ts`, `main.ts` | Dogs feel different to play | ✅ DONE — each trait now maps to specific threat types via DOG_TRAIT_THREAT_BONUS (Speed→timing/sneak, Brave→combat, Happiness→comfort, Sniff→timing/comfort, Compact→sneak/combat), trait bonus amplified by 50% of modifier diff on matching threats |
| 7.7 | **Zone connectivity verification** — add entrance gates for neighborhood/home chains, verify all 18 zones reachable via BFS | `data.ts`, `src/connectivity.test.ts` | No dead zones | ✅ DONE — added returnZone to apartment/shelter/neighborhood/home/garden/library/market/cave, added shelter_to_neighborhood and neighborhood_home entrance rooms, connectivity test passes (365 tests) |
| 7.8 | **Playwright E2E test** — navigate all zones end-to-end | `e2e/` | Visual regression catch | TODO |
