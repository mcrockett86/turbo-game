...
export class VisualEffectsRenderer {
  ...
  /** Celebration burst — gold/confetti particles rising from center */
  spawnCelebration(cx: number, cy: number, count: number = 80): void {
    const colors = ['#ffd700', '#ff6b35', '#ff4081', '#00e5ff', '#76ff03', '#ff9100'];
    for (let i = 0; i < count && this.particles.length < MAX_PARTICLES; i++) {
  ...
  /** Pickup sparkle — burst of small white/gold sparkles */
  spawnPickupSparkles(x: number, y: number, count: number = 20): void {
    for (let i = 0; i < count && this.particles.length < MAX_PARTICLES; i++) {
  ...
  /** Zone entrance burst — expanding ring of particles */
  spawnZoneEntrance(cx: number, cy: number, count: number = 40, color: string = '#00e5ff'): void {
    for (let i = 0; i < count && this.particles.length < MAX_PARTICLES; i++) {
